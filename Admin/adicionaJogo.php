...
?>
            ...
            <form action="confirmaNovoJogo.php" method="post" enctype="multipart/form-data">
                <tr class="active" data-number="1" style="background-color: #DEE2E6">
                    <td>
                        ...
                        ?>
                        <select id="select" name="jogoCasaClubeId" style="cursor: text; width: 170px; height: 50px">
                            <?php
                    ...
                    <td><a class="nolink"></a><input class="btn btn-outline" type="text" onkeyup="this.value=this.value.replace(/[^\d]/,'')"  name="jogoCasaGolos" style="cursor: text; width: 170px; height: 50px"></td>
                    <td>
                        <?php
                        $sql="select * from clubes";
                        $res=mysqli_query($con,$sql);
                        ?>
                        <select id="select2" name="jogoForaClubeId" style="cursor: text; width: 170px; height: 50px">
                            <?php
                            while($dados=mysqli_fetch_array($res)){
                                ?>
                                <option value="<?php echo $dados['clubeId']?>"><?php echo $dados['clubeNome']?></option>
                    ...
                    <td><a class="nolink"></a><input class="btn btn-outline" onkeyup="this.value=this.value.replace(/[^\d]/,'')"  type="text" name="jogoForaGolos" style="cursor: text; width: 170px; height: 50px"></td>
    ...
    <script>
        ...
        let select2Options = select2.getElementsByTagName('option');


        select.addEventListener('change', (event) => {
            for(var i = 0; select2Options.length; i++){
                if(event.target.value === select2[i].value){
